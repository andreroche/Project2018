...
# For ease of review, each block will be named and will correspond with learnings in the Final Feport.
# Credit to Data School (https://www.youtube.com/channel/UCnVzApLJE2ljPZSeQylSEyg) and (www.dataschool.io) for vast resourses on machine learning. These guys are awesome!
...
data = numpy.genfromtxt('data/iris_Dataset.csv', delimiter =',') # import dataset, genfromtxt function used to create arrays from tabular data.
from scipy import stats # import the scipy library , from that import the statistics module.
from scipy.stats import skew, kurtosis, normaltest  # from the statistics module, import skew, kurtosis and normaltest classes.
...
# Using pandas library to import and print the Iris Dataset on-screen.
...
print(df.describe()) # print the stat summary of dataframe or df.
#print(df.skew())
#print(df.kurtosis())
print('')
...
print('')
print(df.groupby('species').corr()) # Printing a correlation between features by species.
print('')

df.groupby('species').boxplot() # boxplot of features by species.
plt.show()

 # Histograms of features by species and print to screen / plot.
...
pd.plotting.scatter_matrix(df) # plot the scatter matrix
plt.show()
...
from pandas import scatter_matrix # import the scatter_matrix module from pandas.
import seaborn as sns # import the seaborn library as sns alias.
import matplotlib.pyplot as plt  # import matplot.pyplot as plt alias
from scipy import stats # from scipy import the stats module
from scipy.stats import skew, kurtosis, normaltest # from the scipy.stats module import the skew, kurtosis and normal test classes.
...
sns.set(style = "ticks", color_codes = True) # set parameters
iris = sns.load_dataset("iris")
p = sns.pairplot(iris) # create a pairplot called p
plt.show() # show it on screen

import seaborn as sns
sns.set(style = "ticks", color_codes = True)
iris = sns.load_dataset("iris")
d = sns.pairplot(iris, hue = "species") # note the hue setting by species makes a nice graph!
plt.show()

import seaborn as sns
sns.set(style = "ticks", color_codes = True)
iris = sns.load_dataset("iris")
f = sns.pairplot(iris, hue = "species", kind = "reg") # note - add regression to the features. possible idea with regression, know slope and intercept, create correlation offsets between line
plt.show()

# Block 4  - Classification methods and machine learning.

from sklearn.datasets import load_iris # import the load iris function from sklearn datasets.

iris = load_iris() # run the load_iris function.

type (iris) # save the return value in a object called iris.
...
print (iris.target) # print the target values - either a 0,1 or 2 (the species that i want to predict)
print('')
print ('The species names are as follows:')
print (iris.target_names) # print the names of the targets( the species).

print('')
print (type(iris.data)) # what type is the data (it should be an numpy array). These are the features.
print (type(iris.target)) # print what type is the target (it should be a numpy array). This is the response (species).
...
X = (iris.data) # X is to store the data (the feature data). X represents a matrix.
y = (iris.target) # y is to store the target (the species). y represents a vector.
...
knn= KNeighborsClassifier(n_neighbors=1) # define object called knn that can calculate k-nearest neightbors and use K = 1

print (knn) # print knn 

knn.fit(X,y) # fit the model with data. this is the training step. 
X_old = ([[1,2,3,4]]) # an iris flower (feature) set
...
X_new = ([[3,5,4,2], [5,4,3,2]]) # two iris flowers (two features sets)
knn.predict (X_new)
...
knn.predict(X_new) # predict for X_new
...
print ('Where: 0 = Setosa, 1 = Versicular, 2 = Virginica ') # This is the key, the responses are assiged numerics. just printing them to screen.